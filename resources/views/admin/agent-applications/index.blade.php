...
<x-admin-app-layout>
    <x-section-div-style>
        <!-- admin/agent-applications/index.blade.php -->
            ...
            @forelse ($applications as $application)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-2 py-1 text-sm text-gray-700 border-b">
                        <!-- Approve Form -->
                        <form action="{{ route('admin.agent.approve', $application->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('POST')
                            <button 
                                type="submit"
                                class="px-2 py-1 text-sm font-semibold text-white bg-green-600 rounded hover:bg-green-700">
                                <i class="fa-solid fa-check"></i>
                            </button>
                        </form>
                        
                        <!-- Reject Form -->
                        <form action="{{ route('admin.agent.reject', ['applicationId'=>$application->id]) }}" method="POST" class="inline-block ml-2">
                            @csrf
                            @method('POST')
                            <button 
                                type="submit"
                                class="px-2 py-1 text-sm font-semibold text-white bg-red-600 rounded hover:bg-red-700">
                ...
                <tr>
                    <td colspan="6" class="px-4 py-6 text-center text-sm text-gray-500">
                        <div class="bg-yellow-100 border border-yellow-300 p-4 rounded-md">
                            <strong class="font-semibold text-yellow-700">No applications found.</strong> 
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-white">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div class="flex-1">
                        <h2 class="text-xl font-semibold text-gray-800">Agent Application</h2>
                        <p class="mt-1 text-sm text-gray-600">Approve pending applications</p>
                    </div>
